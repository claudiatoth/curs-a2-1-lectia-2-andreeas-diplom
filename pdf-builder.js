// PDF BUILDER — Andreeas Diplom (A2.1 · Lecția 2)
(function () {
    ...
    function buildTheory() {
        ...
        return `<h1 class="chapter">📘 1. Teorie — Andreeas Diplom: WEIL + DASS + Modalverbe la Präteritum + Imperativ sein</h1>` + t;
    }
    ...
    function rfBlock(title, instruction, data) {
        let h = `<div class="ex-block"><h3>${title}</h3><div class="instruction">${instruction}</div><div class="rezolvare-banner">✓ Rezolvare</div>`;
        data.forEach((it, i) => {
            const mark = it.correct === 'richtig' ? '✅ <strong style="color:#047857">RICHTIG</strong>' : '❌ <strong style="color:#dc2626">FALSCH</strong>';
            h += `<div class="ex-item"><span class="ex-num">${i + 1}</span><div class="ex-body"><div class="ex-q">${it.text}</div><div class="ex-a">${mark}${it.expl ? ' — ' + it.expl : ''}</div></div></div>`;
        });
        return h + `</div>`;
    }

    function transformBlock(title, instruction, data) {
        let h = `<div class="ex-block"><h3>${title}</h3><div class="instruction">${instruction}</div><div class="rezolvare-banner">✓ Rezolvare</div>`;
        data.forEach((it, i) => {
            h += `<div class="ex-item"><span class="ex-num">${i + 1}</span><div class="ex-body"><div class="ex-q"><strong>${it.prompt}</strong>${it.translation ? `<br><em style="color:#6b7280; font-size:9.5pt;">🇷🇴 ${it.translation}</em>` : ''}</div><div class="ex-a">${it.correct}</div></div></div>`;
        });
        return h + `</div>`;
    }
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += rfBlock('Übung 1 — Adevărat sau Fals (povestea Andreeei)', 'Citește povestea din secțiunea Teorie 1 și bifează adevărat / fals.', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += transformBlock('Übung 2 — Propoziții cu WEIL (transformare 2→1)', 'Combină cele 2 propoziții într-una cu „weil". Virgulă ÎNAINTE de weil + verbul la SFÂRȘIT.', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += transformBlock('Übung 3 — Vorbire indirectă cu DASS (Indirekte Rede)', 'Transformă vorbirea directă în indirectă. Schimbă pronumele (ich → er/sie) + verbul la SFÂRȘIT.', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += fillInBlock('Übung 4 — Modalverbe la Präteritum (musste, wollte, durfte, konnte, sollte)', 'Rădăcină + -te/-test/-te/-ten/-tet/-ten. Atenție: dürfen/können/müssen pierd Umlaut-ul.', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += translateBlock('Übung 5 — Traducere RO → DE (mix weil + dass + Imperativ)', 'Atenție la topică: verbul pe poziția 2 în Hauptsatz, verbul la SFÂRȘIT în Nebensatz (cu weil/dass).', ex5Data);
        }
        return html;
    }
